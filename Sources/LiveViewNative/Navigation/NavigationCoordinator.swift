...
class NavigationCoordinator<R: CustomRegistry>: ObservableObject {
    let initialCoordinator: LiveViewCoordinator<R>
    @Published var navigationPath = [URL]()
    var coordinators = [LiveViewCoordinator<R>]()
    
    var cancellables = Set<AnyCancellable>()
    
    @MainActor
    init(initialCoordinator: LiveViewCoordinator<R>) {
        ...
        self.$navigationPath.sink { [weak self] path in
            ...
            for url in path {
                if let coordinator = self?.coordinator(for: url) {
                    ...
                    let coordinator = LiveViewCoordinator<R>(url, config: config)
    ...
    @MainActor
    func coordinator(for url: URL) -> LiveViewCoordinator<R>? {
