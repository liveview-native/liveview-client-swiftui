...
struct LiveRedirect {
    ...
    enum Mode: String {
        ...
        ///
        /// 1. If the mode is `push`, a new `LiveViewCoordinator` is created for the redirect destination.
        /// A separate Phoenix channel is connected for each coordinator.
        /// 2. If the mode is `redirect` and the destination is the same as the previous path, the current entry is popped and no new `LiveViewCoordinator` is connected.
