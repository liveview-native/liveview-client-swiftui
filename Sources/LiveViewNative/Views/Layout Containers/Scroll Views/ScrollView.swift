    ...
    func body(content: Content) -> some View {
        content
            .onScrollGeometryChange(for: CGPoint.self, of: { geometry in
                ...
                context.coordinator.session.scrollPositions[context.coordinator.session.navigationPath.count, default: [:]][id] = newValue
            })
            .scrollPosition($position)
            .task {
                // restore the scroll position from the ``LiveSessionCoordinator``
                defer { didAttemptRestoration = true }
                guard let restoredValue = context.coordinator.session.scrollPositions[context.coordinator.session.navigationPath.count, default: [:]][id]
                else { return }
