...
struct ColorView<Library: ElementLibrary>: View {
    let node: Node
    
    /// The alpha channel of the color, in the range `0` to `1`.
    @_documentation(visibility: public)
    private var opacity: Double {
        node.attributeValue(for: "opacity", strategy: .number) ?? 1
    }
    ...
    /// * `displayP3`
    @_documentation(visibility: public)
    private var colorSpace: SwiftUI.Color.RGBColorSpace {
        node.attributeValue(for: "colorSpace", strategy: RGBColorSpaceParseStrategy()) ?? .sRGB
    }
...
struct RGBColorSpaceParseStrategy: ParseStrategy {
    func parse(_ value: String) throws -> SwiftUI.Color.RGBColorSpace {
        switch value {
        case "displayP3":
            return .displayP3
        case "sRGB":
...
extension SwiftUI.Color.RGBColorSpace: Decodable {
