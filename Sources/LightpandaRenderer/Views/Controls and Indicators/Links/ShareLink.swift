...
struct ShareLink<Library: ElementLibrary>: View {
    let node: Node
    
    /// The title to use when sharing to a service with a subject field.
    @_documentation(visibility: public)
    private var subject: String? {
        node.attributeValue(for: "subject")
    }
    
    /// The description to use when sharing to a service with a message field.
    @_documentation(visibility: public)
    private var message: String? {
        node.attributeValue(for: "message")
    }
    
    /// A JSON-encoded list of strings to share.
    @_documentation(visibility: public)
    private var items: [String]? {
        node.attributeValue(for: "items", strategy: ItemsParseStrategy())
    }

    struct ItemsParseStrategy: ParseStrategy {
    ...
    public var body: some View {
        #if !os(tvOS)
        let useDefaultLabel = !node.children.contains(where: {
            $0.name == "SharePreview"
        })
    ...
    private func previews(for items: [String]) -> PreviewBranch<String>? {
        ...
        var previews: ([String: PreviewData], default: PreviewData?) = node.children
            .filter({ $0.name == "SharePreview" })
