...
//    @ChangeTracked(attribute: "sortOrder") private var sortOrder = TableColumnSortContainer(value: [])
    ...
    private var rows: [TableRow] {
        node.children
            ...
            .flatMap(\.children)
            .compactMap { $0.name == "TableRow" && $0.attributes.keys.contains("id") != nil ? TableRow(element: $0) : nil }
    }
    
    private var columns: [TableColumn<TableRow, TableColumnSort, some View, SwiftUI.Text>] {
        let columnElements = node.children
            .filter { $0.attributeValue(for: "template") == "columns" }
            .filter { $0.name == "TableColumn" }
        return columnElements.enumerated().map { item in
            TableColumn(item.element.children.first?.value ?? "", sortUsing: TableColumnSort(id: item.element.attributeValue(for: "id") ?? String(item.offset), order: .forward)) { (row: TableRow) in
